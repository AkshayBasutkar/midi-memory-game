...
const generateTilesForLayer = (layerIndex: number, gridSize: number): Tile[] => {
  ...
  for (let i = 0; i < pairCount; i++) {
    const iconId = (i % FLOWER_COUNT) + 1;
    iconIds.push(iconId, iconId);
  }
  
  for (let i = iconIds.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));
    [iconIds[i], iconIds[j]] = [iconIds[j], iconIds[i]];
  }
  ...
  for (let row = 0; row < gridSize; row++) {
    for (let col = 0; col < gridSize; col++) {
      ...
      tiles.push({
        ...
        layerIndex,
        position: [x, y, z],
        iconId: iconIds[tileIndex],
        isFlipped: false,
        isMatched: false,
        isActive: layerIndex === 0,
      });
      tileIndex++;
    }
  }
...
export const useMemoryGame = create<GameState>()(
  subscribeWithSelector((set, get) => ({
    ...
    flipTile: (tileId: string) => {
      ...
      if (state.flippedTiles.includes(tileId)) {
        return;
      }
    ...
    checkMatch: () => {
      ...
      const isMatch = tile1.iconId === tile2.iconId;
      console.log(`Match check: ${tile1.iconId} vs ${tile2.iconId} = ${isMatch}`);
    ...
    clearLayer: (layerIndex: number) => {
      ...
      const isLastLayer = nextLayerIndex >= state.layers.length;
      
      const newLayers = state.layers.map((layer, index) => {
        if (index === layerIndex) {
          return { ...layer, isCleared: true, isActive: false };
        }
      ...
      const clampedLayerIndex = Math.min(nextLayerIndex, state.layers.length - 1);
